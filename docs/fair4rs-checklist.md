...
| FORRT Outcome's `Repository URL` field | Points to `https://github.com/annefou/fiesta-galaxy-scattering-bioimaging`. |
...
| GHCR Docker pull | `docker pull ghcr.io/annefou/fiesta-galaxy-scattering-bioimaging:latest` over HTTPS. |
...
- [ ] Docker image builds and runs the snakemake target via `docker run --rm ghcr.io/annefou/fiesta-galaxy-scattering-bioimaging:latest`.
